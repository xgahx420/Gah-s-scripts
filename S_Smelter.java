...
public final class S_Smelter extends Script
    ...
    public int main() {
        ...
        if (isBanking()) {
            ...
            if (ic > ncount) {
                System.out.println("Sanity check primary ore");
                deposit(bar.ore_id, ic - ncount);
                return 1200;
            }
            int coalCount = getInventoryCount(bar.coal_id);
            if (coalCount > ncount*bar.coal_count) {
                System.out.println("Sanity check coal/secondary");
                deposit(bar.coal_id, coalCount - ncount*bar.coal_count);
                return 1200;
            }
            if (ic <= 0) {
                int w = ncount - ic;
